...
import { PPCAdGroup } from '@/types/ppcDashboard.types'
...
export const AdGroupTable: React.FC<AdGroupTableProps> = ({ items, isLoading, error }) => {
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
  ...
  if (error) {
    return (
      <Card>
        ...
        <CardContent>
          <div className="text-danger-600 text-sm">Failed to load ad groups.</div>
        </CardContent>
      </Card>
    )
  }
  ...
  if (!items || items.length === 0) {
    return (
      <Card>
        ...
        <CardContent>
          <div className="text-text-muted text-sm">No ad group data available.</div>
        </CardContent>
      </Card>
    )
  }
          ...
          <TableBody>
            {items.map((item) => (
              <TableRow key={item.id}>
                <TableCell className="font-medium">{item.adGroupName}</TableCell>

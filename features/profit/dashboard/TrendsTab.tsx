...
export const TrendsTab: React.FC<TrendsTabProps> = ({
  ...
  // Applied states (passed to TrendsComponent & triggers API refetches)
  ...
  const handleDraftMarketplacesChange = useCallback(
    (value: string[]) => {
      ...
      if (value.length > 0) {
        const autoCurrency = getDefaultCurrencyForMarketplaces(
          value,
          draftCurrency,
        )
  ...
  return (
    <>
      ...
      <TrendsComponent
        ...
        startDate={appliedDateRange.startDate as any}
        endDate={appliedDateRange.endDate as any}
        periodicity={appliedDateRange.periodicity as any}
        searchTerm={searchTerm}
      />
    </>
  )

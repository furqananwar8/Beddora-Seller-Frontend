'use client'

import React from 'react'

export interface TestimonialCardProps {
  quote: string
  authorName: string
  authorTitle: string
  authorInitials?: string
  rating?: number
}

export const TestimonialCard: React.FC<TestimonialCardProps> = ({
  quote,
  authorName,
  authorTitle,
  authorInitials,
  rating = 5,
}) => {
  return (
    <div className="bg-primary-800/60 rounded-2xl p-5 border border-primary-700/50">
      <div className="flex items-center gap-1 mb-3">
        {[...Array(rating)].map((_, i) => (
          <svg key={i} className="w-5 h-5 text-warning-400" fill="currentColor" viewBox="0 0 20 20">
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
          </svg>
        ))}
      </div>
      <p className="text-text-inverse text-base mb-4 italic leading-relaxed">&quot;{quote}&quot;</p>
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-full bg-secondary-700 flex items-center justify-center text-text-inverse font-bold text-xs">
          {authorInitials || authorName.split(' ').map(n => n[0]).join('').toUpperCase()}
        </div>
        <div>
          <p className="font-bold text-text-inverse text-sm">{authorName}</p>
          <p className="text-xs text-secondary-200 mt-0.5">{authorTitle}</p>
        </div>
      </div>
    </div>
  )
}

